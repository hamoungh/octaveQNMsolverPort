...
function [U R Q X] = qncmmva_cs( N, S, P, r, m )
  
  if ( nargin < 3 || nargin > 5 )
   % print_usage();
  end % if

  if  not(isvector(N) && all( N>=0 ))
      error( 'N must be >=0' ); end; 
  N = N(:)'; % make N a row vector
  C = length(N); %% Number of classes
  if  not( ndims(S) == 2 )
      error( 'S must be a matrix' ); end; 
  K = size(S,2); %% Number of service centers
  if not(size(S) == [C,K] )
      error( 'S size mismatch (is %dx%d, should be %dx%d)', rows(S), columns(S), C, K ); end; 
  if not(ndims(P) == 4 & size(P) == [C,K,C,K]) 
      error( 'P size mismatch (should be %dx%dx%dx%d)',C,K,C,K ); end; 
  ...
  if ( nargin < 4 )
   ...
   if  not( isvector(r) && length(r) == C ) 
        error('r must be a vector with %d elements', C); end; 
    r = r(:)';
    if not(all( r>=1 && r<=K ))
        error('elements of r are out of range [1,%d]',K); end; 
   end %if
  ...
  if not(all(S(:) >= 0)) 
      error( 'S must be >= 0' ); end; 
  if not(all( any(S>0,2) )) 
      error( 'S must contain at least a value >0 for each row' ); end; 
 if not( all(P(:)>= 0)) 
      error( 'P must be >=0' ); end; 
  ...
  [Ustar Rstar Qstar Xstar Qnm1] = qncmmva_nocs( Nstar, Sstar, Vstar, m, zeros(size(Nstar)) );
...
%%
function [U R Q X Qnm1] = qncmmva_nocs( N, S, V, m, Z )
  ...
  for k=find(m>1)
    if not(all( S(:,k) == S(1,k) ))
        error( 'Service times for FCFS node %d are not class-independent', k ); end; 
  end
  ...
  N2=(N+1); 
  bufsize = prod(N2(1:end-1)); 
  Q_next = zeros( bufsize,K );
  Q = zeros( bufsize,K );
  ...
  for k=find(m>1)
    ...
    p{k}(1,getidx(N,0*N)) = 1;
  end

  Qnm1= zeros(C,K); %% Qnm1(c,k) is the number of requests in center k, provided that the population size is N-1_c (N is the total population vector). This value is needed by qncmmva_cs. Qnm1 is only filled for M/M/1 or PS centers. The values are not computed for PS nodes
...
%% of N.
function idx = getidx( N, i )
  if ( length(N) == 1 )
    idx = 1;
  else
    i2=i +1; 
  %  i_cell = num2cell( i2(1:end-1) );  
    i_cell = num2cell( i2(1:end) );  
    N2=N+1; 
    % idx = sub2ind( N2(1:end-1), i_cell{:} ); 
     idx = sub2ind( N2(1:end), i_cell{:} ); 
  end %if  
end %function
...
%!      0.05 0.09 0.10 0.08];
%! [U1 R1 Q1 X1] = qncmmva(N,S,V); % this invokes qncmmva_nocs
%! [U2 R2 Q2 X2] = qncmmva(N,S,P); % this invokes qncmmva_cs
%! assert( U2, U1, 1e-5 );
%! assert( R2, R1, 1e-5 );
%! assert( Q2, Q1, 1e-5 );
%! assert( X2, X1, 1e-5 );
