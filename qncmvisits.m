...
function [V chains] = qncmvisits( P, r )
  ...
  if not(ndims(P) == 4 )
      error('P must be a 4-dimensional matrix'); end; 

  [C, K, C2, K2] = size( P );
  if not(K == K2 && C == C2) 
      error( 'P must be a [C,K,C,K] matrix'); end; 
  ...
  if ( nargin < 2)
    ...
    if not( all( r>=1 & r<=K ) )
        error('elements in r must be in the range 1 - %d',K); end; 
    r = r(:)';
  end
  ...
  for k=1:K
    for c=1:C
      if ( chains(c) == 0 )
        ...
        if not( CH(c,k) == 0 || chains(c) == CH(c,k) ) 
            error('Class %d belongs to different chains',c); end; 
      end
    end %for
  end %for
...
function v = dfs(G, s)
  % assert( issquare(G) );
  N = size(G,1);
  v =  zeros(1,N); %% v(i) == 1 iff node i has been visited  
  stack = zeros(1,N);    
