...
function [U R Q X] = qncmmvaap( N, S, V, m, Z, tol, iter_max )
  ...
  if not(isvector(N) && all( N>=0 )) error( 'N must be a vector of positive integers' ); end;
  N = N(:)'; % make N a row vector
  C = size(N,2); %% Number of classes
  K = size(S,2); %% Number of service centers
  if size(S) ~= [C,K]  error( 'S size mismatch' ); end;
  if size(V) ~= [C,K]  error( 'V size mismatch' ); end;

  if ( nargin < 4 || isempty(m) )
    m = ones(1,K);
  else
    if not(isvector(m)) error( 'm must be a vector'); end;
    m = m(:)'; % make m a row vector
    if not( length(m) == K && all( m <= 1 ) )  error( 'm must be <= 1 and have %d elements', K ); end; 
  end

  if ( nargin < 5 || isempty(Z) )
    Z = zeros(1,C);
  else
    if not(isvector(Z)) error( 'Z must be a vector' ); end;
    Z = Z(:)'; % make Z a row vector
    if not( length(Z) == C && all(Z >= 0 )) error( 'Z must be >= 0 and have %d elements', C ); end;   
  end
  ...
  if  not( all(S(:) >= 0))   error( 'S contains negative values' ); end;
  if not(all(V(:) >= 0)) error( 'V contains negative values' ); end;
  ...
  while(1) % do
    ...
    err = norm((Q-Qold)./Qold, 'inf');
       ...
       if (err<tol || iter>iter_max) % until (err<tol || iter>iter_max);
            break;
        end;
  end;
